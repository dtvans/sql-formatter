...
public class Tokenizer {
    ...
    public Tokenizer(DialectConfig cfg) {
        ...
        this.PLACEHOLDER_PATTERN =
                Pattern.compile(RegexUtil.createRegex(new JSLikeList<>(cfg.placeholders)));
    }
    ...
    private Token getPlaceHolderToken(String input) {
        return this.getTokenOnFirstMatch(input, TokenTypes.PLACEHOLDER, this.PLACEHOLDER_PATTERN);
    }
    ...
    private Token getPlaceholderTokenWithKey(
        ...
        Token token = this.getTokenOnFirstMatch(input, TokenTypes.INDEX_PLACEHOLDER, regex);
        if (token != null) {
